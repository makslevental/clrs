...
it at the head. Running time is $O\left(n^{2}\right)$.Running time
is $O\left(n^{2}\right)$.
\begin{algorithm}[H]
\noindent \begin{raggedright}
...
if len$\left(A\right)==1$:
	...
	while len$\left(L\right) > 0 $ and len$\left(R\right) > 0$:
		...
		if $L\left[1\right] < R\left[1\right]$:
			...
			del $R\left[1\right]$
	# if one of $L,R$ is larger by one element.
	if len$\left(L\right) > 0 $
		$M$.append$\left(L\left[1\right]\right)$
	elif len$\left(R\right) > 0 $
		$M$.append$\left(R\left[1\right]\right)$: 
	return $M$
\end{lstlisting}
\end{algorithm}
...
$n$ \texttt{Extract-Max }operations on a $k$ length priority queue\texttt{.}
\begin{algorithm}[H]
\noindent \begin{raggedright}
...
for $i=k+1:\text{len}\left(A\right)$: 
	$u \sim \text{Uniform}\left(0,1\right)$
	# $H$.max returns value of maximum without extracting
	if $u < H$.min:
		$H$.Extract-Max$\left(\right)$		
		$H\text{.insert}\left(u,A\left[i\right]\right)$
\end{lstlisting}
\end{algorithm}
...
\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
$m = A\left[1\right]$
# these selections don't count against the quota
...
nodes. Note that given the suggested distribution of multilevel nodes
half will be level-1, $1/4$ will be level-2, $1/8$ will be level-3,
etc. Here are three skip lists which contain level-1, level-2, level-4,
and level-8 nodes, i.e. that implement skipping every 2, 4, and 8
nodes:

\begin{figure}[H]
\noindent \begin{centering}
\includegraphics[scale=0.4]{skip_lists}
\par\end{centering}

\end{figure}


Search in such data structures is $O\left(\lg n\right)$ worst case.
To implement there should be a header node that has links to the next
nodes of each extant height and all the trailing nodes should have
a link to a terminating node whose ``value'' is greater than all
possible values. 

How do you search in such a data structure? Start at the ``highest
level'' and keep going forward (along the same ``level'') until
the next node is an overshoot, then go down a level and repeat until
you either find the key or get to the bottom.

\begin{algorithm}[H]
\noindent \begin{raggedright}
\texttt{SearchV1}$\left(list,key\right)$
\par\end{raggedright}

\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
# every level-$k$ node is a dict with a list of pointers,
# the key, and the value associated with the node
# $list\left[\text{'header'}\right]$ is a dict with just the
# list of pointers
$node = head = list\left[\text{'header'}\right]$
for $i=\text{len}\left(head\left[\text{'pointers'}\right]\right):1$:
	# $node\left[\text{'pointers'}\right]\left[i\right]$ is a pointer to the next node
	while $node\left[\text{'pointers'}\right]\left[i\right]\left[\text{'key'}\right]<key$:
		$node=node\left[\text{'pointers'}\right]\left[i\right]$
$node=node\left[\text{'pointers'}\right]\left[1\right]$
if $node\left[\text{'key'}\right] == key$:
	return $node\left[\text{'val'}\right]$
else:
...
As written this data structure is great for searching, but insertion
and deletion would be impractical (difficult to maintain the regularity
of links between every second, every fourth, every eighth, every sixteenth,
etc.). What's the solution? Simply make the height of inserted nodes
geometric random\footnote{level-1 with $p=\frac{1}{2}$, level-2 with $p=\frac{1}{4}$,level-4
with $p=\frac{1}{8}$, etc.}, in order to preserve the same proportions of level-$k$ nodes as
mentioned above. A close analysis\footnote{Pugh, W. (1990). \textquotedbl{}Skip lists: A probabilistic alternative
to balanced trees\textquotedbl{}. \emph{Communications of the ACM}
\textbf{33} (6)} shows that this yields \emph{expected} $O\left(\lg n\right)$ bounds
instead of deterministic $O\left(\lg n\right)$ bounds. 

Since we're now inserting nodes the header should also able to accomodate
insertion of nodes with higher level than currently in the list; there
should a \emph{MaxLevel} of the list and the header node should have
that many links (where for non-extant heights they should point directly
to the terminator).


\subsection{Search}

How does the fact that inserted nodes are of random height affect
search? Could we end up with too many tall nodes? No. The probability
that for a list of length $n$ the highest node is greater than $\log_{-\frac{1}{2}}\left(n\right)$
is small. Furthermore, given this, we can safely cap $MaxLevel=\log_{-\frac{1}{2}}\left(n\right)$
where $n$ is the anticipated size of the list. For $n=2^{16}$ we
get $MaxLevel=16$. Note that we don't need to start searching at
$MaxLevel$, we only need to start searching at the level of the highest
node in the list (which we should keep track of therefore). As mentioned
expected running time is $O\left(\lg n\right)$.

\begin{algorithm}[H]
\noindent \begin{raggedright}
\texttt{SearchV}2$\left(list,key\right)$
\par\end{raggedright}

\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
$node = list\left[\text{'header'}\right]$
for $i=list\left[\text{'currentlvl'}\right]:1$:
	# just a small optimization
	if $node\left[\text{'pointers'}\right]\left[i\right]\left[\text{'key'}\right] == key$:
		break
	else:
		while $node\left[\text{'pointers'}\right]\left[i\right]\left[\text{'key'}\right]<key$:
			$node=node\left[\text{'pointers'}\right]\left[i\right]$
...
As mentioned we can insert by generated a random height node, but
we need to update pointers for all other nodes that are were used
to ``climb down'' to the insert node. What about if we generate
a node that's much taller than the current height of the list? Theoretically
it doesn't matter (as mentioned this won't happen often enough to
matter) but practically speaking we can just always truncate to the
current highest level +1. As mentioned expected running time is $O\left(\lg n\right)$.

\begin{algorithm}[H]
\noindent \begin{raggedright}
\texttt{Insert}$\left(list,key,value\right)$
\par\end{raggedright}

\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
$node = head = list\left[\text{'header'}\right]$
# we'll use this list to keep track which nodes need to be updated
$maxlvl = list\left[\text{'maxlvl'}\right]$
$update = maxlvl \cdot\left[\text{Terminator}\right]$
for $i=head\left[\text{'currentlvl'}\right]:1$:
	while $node\left[\text{'pointers'}\right]\left[i\right]\left[\text{'key'}\right]<key$:
		$node=node\left[\text{'pointers'}\right]\left[i\right]$
	$update\left[i\right] = node$
$node=node\left[\text{'pointers'}\right]\left[i\right]$
# random$\left(\right)$ is uniform random $\left[0,1\right]$ and
# therefore $ \frac{\ln\left(\text{random}\left(\right)\right)}{\ln\left(1-\frac{1}{2}\right)}$ is geometric
$lvl =\min\left\{ maxlvl+1,\left\lfloor \frac{\ln\left(\text{random}\left(\right)\right)}{\ln\left(1-\frac{1}{2}\right)}\right\rfloor \right\} $
if $lvl > list\left[\text{'currentlvl'}\right]$:
	$list\left[\text{'currentlvl'}\right] = lvl$
$x = \left\{ \text{'pointers'}:lvl\cdot\left[\text{None}\right],\text{'key'}:key,\text{'val'}:value\right\}$ 
# splice in $x$, i.e. fix all pointers
for $i = 1:lvl$:
	$x\left[\text{'pointers'}\right]\left[i\right]=update\left[i\right]\left[\text{'pointers'}\right]\left[i\right]$
	$update\left[i\right]\left[\text{'pointers'}\right]\left[i\right] = x$	
\end{lstlisting}
\end{algorithm}
...
Deletion works pretty similarly. As mentioned expected running time
is $O\left(\lg n\right)$.

\begin{algorithm}[H]
\noindent \begin{raggedright}
\texttt{Delete}$\left(list,key\right)$$ $
\par\end{raggedright}

\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
$node = head = list\left[\text{'header'}\right]$
# we'll use this list to keep track which nodes need to be updated
$currentlvl = list\left[\text{'currentlvl'}\right]$
$update = currentlvl \cdot\left[\text{Terminator}\right]$
for $i=head\left[\text{'currentlvl'}\right]:1$:
	while $node\left[\text{'pointers'}\right]\left[i\right]\left[\text{'key'}\right]<key$:
		$node=node\left[\text{'pointers'}\right]\left[i\right]$
	$update\left[i\right] = node$
...
if $node\left[\text{'key}\right]==key$:
	for $i=1:currentlvl$:
		# if the staircase node doesn't point to the node to be deleted
		# then don't change anything
		if $update\left[i\right] \neq node$:
			break
		else:
			$update\left[i\right]\left[\text{'pointers'}\right]\left[i\right] = x\left[\text{'pointers'}\right]\left[i\right]$	
		del $x$
		# if $x$ was the tallest node then decrement current level
		while $list\left[\text{'currentlvl'}\right] >1$ and $head\left[\text{'currentlvl'}\right] == \text{Terminator}$:
			$list\left[\text{'currentlvl'}\right] = list\left[\text{'currentlvl'}\right] -1 $	
\end{lstlisting}
\end{algorithm}
...
An interval tree is built atop your favorite balanced binary tree
...
out the highest floor \emph{an} egg can be dropped from safely. The
assumptions are 
\begin{itemize}
...
\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
$V,E = G$
# initialize all vertices to have $v\left[\text{'key'}\right] =\infty$ and
...
and correspond to the minimum cut. To prove this we need to basically
almost prove the Min cut - Max Flow theorem
\begin{proof}
...
\subsection{Voronoi diagram\protect\footnote{Most of this comes from David Mount's http://www.cs.umd.edu/class/fall2014/cmsc754/Lects/cmsc754-fall14-lects.pdf.
Some also from David Austin http://www.ams.org/samplings/feature-column/fcarc-voronoi.}}
...
\subsubsection{\textbf{Fortune's algorithm}}

Fortune's algorithm is a sweep-line algorithm (like line-segment pair
intersection): it uses a sweep-line and an event queue. It also uses
as a conceptual tool something called the ``beach line''. 

First a preliminary: recall that a parabola is defined by a point
called the \textbf{focus} and a line called the \textbf{directrix}\@.
The parabola itself is the set of points equidistant from the focus
and the directrix:
...
\end{figure}
In a way that will be made clear shortly Fortune's algorithm creates
a beach line (because it looks like the bluffs of a beach) by drawing
the parabola created by the sweep-line and each \textbf{site} (points
w.r.t. which we wish to create a Voronoi diagram):

\begin{figure}[H]
\noindent \centering{}\includegraphics[scale=0.4]{beach}
\end{figure}


Each intersection of two adjacent parabolas (the points on the beach
line where the dashed lines - the Voronoi edges - extend from) are
called \textbf{breakpoints}. Here at key insights to understanding
fortune's algorithm:
\begin{enumerate}
\item Breakpoints are created when the sweep-status crosses a site:


\begin{figure}[H]
\noindent \centering{}\includegraphics[scale=0.3]{breakpoints}
\end{figure}



Note that a degenerate parabola (when the focus is on the directrix)
is a line.

\item Breakpoints are equidistant from the sites that the adjacent parabolas
are defined by. Why? For each site, by the definition of the parabola,
the distance to the intersection point on the respective site's parabola
is equal to the distance from the intersection to the sweep-line.
Therefore breakpoints lie on Voronoi edges.
\item Voronoi vertices correspond to the disappearance (shrink to arc-length
0) of parabolas:


\begin{figure}[H]
\noindent \centering{}\includegraphics[scale=0.3]{circle_event}
\end{figure}



When does a parabola in between its two neighboring parabolas disappear?
When all three sites lie on a circle (the circumcircle of the sites)
with center being the Voronoi vertex. Why? When a parabolic arc shrinks
to a point, that point then lies on three parabolas: the one containing
the disappearing arc and the two parabolas containing the arcs on
either side. This means that the point is equidistant from three sites
corresponding to these parabolas and that these three sites therefore
lie on a circle centered at the point. \textbf{And} this happens when
the sweep-line is just tangent to this circumcircle, since the sweep-line
was the directrix of the parabolic arc that disappeared it also intersects
the circumcircle of the three sites (and a line that is a distance
equal to the radius of a circle away from the center of the circle
can only be tangent to that circle). Hence the name circle event.

\end{enumerate}
This suggests an algorithm that consists of a sweep-status, an event
queue, and a DCEL\footnote{Doubly-connected edge list - the standard data structure for representing
planar subdivisions.}. The sweep-line is a balanced binary tree with leaves correspondong
to the ``arcs''. Note we don't need to actually store representations
of the parabolic arcs because we can compute their intersections explicitly
in constant time. Therefore the leaves of the sweep-line are the sites
that define the parabolic arcs, on the beach line and internal nodes
correspond to breakpoints (represented by ordered tuple $\left(p_{i},p_{j}\right)$
where $p_{i}$ is the site corresponding to the parabolic arc to the
left of the breakpoint, and $p_{j}$ is the site corresponding to
the parabolic arc to the right). The event queue is Priority queue
based on decreasing $y$-coordinate: the algorithm first sorts the
coordinates of the sites by decreasing $y$-coordinate and inserts
them into the queue. As the sweep-line ``sweeps'' down it encounters
two types of events: \textbf{site events} and \textbf{circle events}.
Site events are when the sweep-line passes over a new site and the
corresponding ``arc'' is inserted into the beach line. Circle events
are when the length of a parabolic arc shrinks to zero, and a Voronoi
vertex is created in its place. Running time is $O\left(n\lg n\right)$.

Let's look at each event type more closely.
\begin{itemize}
\item Site event: to process a site event determine the ``arc'' that lies
above the new site $p_{i}$. How to figure out which ``arc'' is
directly above a new site? At each internal node compare the $x$-coordinate
of the new site with the $x$-coordinate of the breakpoint: the equation
for a parabola as a function of the focus $p=\left(p_{x},p_{y}\right)$
and position of the sweep-line $\ell_{y}$ is
\[
y=f\left(x,p,\ell_{y}\right)=\frac{\left(x^{2}-2p_{x}x+p_{x}^{2}+p_{y}^{2}-\ell_{y}^{2}\right)}{2\left(p_{y}-\ell_{y}\right)}
\]
Therefore setting $y_{i}=f\left(x,p_{i}\ell_{y}\right)=y_{j}=f\left(x,p_{j}\ell_{y}\right)$
and solving for $x$ gives the $x$-coordinate of the breakpoint.
Then use this comparison to traverse the tree in $O\left(\lg n\right)$
time. 


Upon insertion of a new arc we split the arc above the inserted one.
This is done by updating either the left child or the right child
of the split arc (according to whether the new site is to the left
of the site defining the parabolic arc, or the right). Assume the
new site is to the right. Then leaf corresponding to the split arc
becomes an internal node with value being the intersection of the
new arc and the left half of old arc (computed easily on the fly since
the parabola corresponding to the new site is degenerate), left child
corresponding to the original arc, and right child a subtree. The
value of the root of the subtree is the intersection of the new arc
with the right half of the old arc, left child (a leaf) in the subtree
corresponds to the new arc, and right leaf corresponds to the old
arc. 


We also insert a new (dangling) edge into the DCEL which lies on the
bisector\footnote{Point that is half on the line between.} between
$p_{i}$ and $p_{j}$. Insertion of a new parabolic arc also creates
circle events: triples of the sites neighboring $p_{i}$ define the
circumcenter of the cirumcircle passing through the triple.


Concretely suppose the beach line is $\left\langle p_{1},p_{2},p_{j},p_{3},p_{4}\right\rangle $.
Insertion of $p_{i}$ splits $p_{j}$ into arcs $p_{j}^{'}$ and $p_{j}^{''}$.
The new sequence becomes $\left\langle p_{1},p_{2},p_{j}^{'},p_{i},p_{j}^{''},p_{3},p_{4}\right\rangle $.
Any circle events in the event queue corresponding to $\left\langle p_{2},p_{j},p_{3}\right\rangle $
are deleted (therefore leaf-nodes should keep pointers to them in
the event queue) and new ones corresponding $\left\langle p_{2},p_{j}^{'},p_{i}\right\rangle $
and $\left\langle p_{i},p_{j}^{''},p_{3}\right\rangle $ are inserted.
Note that $\left\langle p_{j}^{'},p_{i},p_{j}^{''}\right\rangle $
does not generate a circle event because it corresponds to only two
distinct sites.

\item Circle event: let $\left\langle p_{i},p_{j},p_{k}\right\rangle $
be the triple corresponding to the circle event. To process the circle
event we delete arc $p_{j}$ from the beach line, create a new vertex
in the Voronoi diagram at the intersection of that parabolic arcs
corresponding to $p_{i},p_{j},p_{k}$, tie the edges for the bisector
between $p_{i}$ and $p_{j}$ and the bisector between $p_{j}$ and
$p_{k}$, and start a new edge for the bisector between $p_{i}$ and
$p_{k}$ that extends from the Voronoi vertex. Finally we delete any
other circle events involving $p_{j}$ and generate new ones corresponding
to the two new triples involving $p_{i}$ and $p_{k}$. 


Concretely suppose the beach line is $\left\langle p_{1},p_{i},p_{j},p_{k},p_{2}\right\rangle $.
After processing the beach line is $\left\langle p_{1},p_{i},p_{k},p_{2}\right\rangle $
and any events associated with $\left\langle p_{1},p_{i},p_{j}\right\rangle $
and $\left\langle p_{j},p_{k},p_{2}\right\rangle $ are removed. We
also consider the creation of new events $\left\langle p_{1},p_{i},p_{k}\right\rangle $
and $\left\langle p_{i},p_{k},p_{2}\right\rangle $. \end{itemize}
