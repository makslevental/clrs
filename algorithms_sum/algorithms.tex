...
Prim's algorithm uses a min heap to keep the sorted list of ``nearest''
vertices and updates approximations of farther vertices as near ones
are explored. It functions very similarly to Dijkstra's shortest path
algorithm (which is covered in the next section.
...
\begin{lstlisting}[basicstyle={\ttfamily},language=Python,mathescape=true,numbers=left,showstringspaces=false,tabsize=3]
$attrs = \{i:\{\text{'dist'}:\infty,\text{'prnt':None}\} \text{ for } i=1:\text{len}\left(G\right) \}$ 
# initialize some vertex to be the nucleation point of the 
# minimum spanning tree
$attrs\left[1\right]\left[\text{'dist'}\right] = 0$
$V,E = G$
# let's pretend this minqueue is keyed on $attrs\left[\text{'dist'}\right]$
...
while $\text{len}\left(Q\right) > 0$:
	...
	for $v \in V\left[u\right]\left[\text{'neighbors'}\right]$:
		...
		# basically just have a bunch of dicts floating around
			if $v \in Q$ and $E\left[\left(u,v\right)\right] < attrs\left[v\right]\left[\text{'dist'}\right]$:
				$attrs\left[v\right]\left[\text{'dist'}\right] = E\left[\left(u,v\right)\right]$
				# crucially you need to be able to adjust the priority key in the min queue
				$attrs\left[v\right]\left[\text{'prnt'}\right] = u$
# minimum spanning tree is parent edges along shortest path
return $\left[ \left(attrs\left[v\right]\left[\text{'prnt'}\right],v\right) \text{ for }v\in V\right]$
\end{lstlisting}
\end{algorithm}
